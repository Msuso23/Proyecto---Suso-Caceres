...
public class Grafo {
    ...
    int getIndex(String name){
        if(users.getpFirst() == null) 
            return -1;
        if(users.getpFirst().getData().getName().equals(name))
            return 0;
        int index = 1;
        Nodo<User> aux = users.getpFirst().getpNext();
        while (aux != null) {
            if(aux.getData().getName().equals(name)){
                return index;
            }
